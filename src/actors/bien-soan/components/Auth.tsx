...
const ROLE = "bien-soan" as const;
const ACCENT = "#1C63EA";
const TITLE = "Biên soạn";
const DEMO = "biensoan1@toeic.vn / 123456";

/** Đăng nhập / Đăng ký cho actor Biên soạn (Content Manager). */
export function Auth({ onAuthed }: { onAuthed: (a: Account) => void }) {
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(160deg,#eaf0fe,#dde7fb)", fontFamily: "'Inter', system-ui, sans-serif", padding: 20 }}>
      <BackToHome /><ZaloButton />
      <form onSubmit={submit} style={{ width: "100%", maxWidth: 380, background: "#fff", borderRadius: 18, padding: 30, boxShadow: "0 16px 44px rgba(28,99,234,0.14)" }}>
        ...
        <p style={{ fontSize: 13, color: "#64748b", marginTop: 5, marginBottom: 20 }}>
          {mode === "login" ? "Đăng nhập để vào xưởng biên soạn nội dung" : "Tạo tài khoản biên soạn mới"}
        </p>
