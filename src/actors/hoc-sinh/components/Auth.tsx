...
const ROLE = "hoc-sinh" as const;
const ACCENT = "#1d4ed8";
const TITLE = "Học sinh";
const DEMO = "hocsinh1@toeic.vn / 123456";

/** Đăng nhập / Đăng ký cho actor Học sinh (đơn giản, theo tông xanh của Học sinh). */
export function Auth({ onAuthed }: { onAuthed: (a: Account) => void }) {
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(160deg,#eef4ff,#dbe7ff)", fontFamily: "'Inter', system-ui, sans-serif", padding: 20 }}>
      ...
      <form onSubmit={submit} style={{ width: "100%", maxWidth: 380, background: "#fff", borderRadius: 18, padding: 30, boxShadow: "0 16px 44px rgba(29,78,216,0.14)" }}>
        ...
        <p style={{ fontSize: 13, color: "#64748b", marginTop: 5, marginBottom: 20 }}>
          {mode === "login" ? "Đăng nhập để vào khu vực học tập" : "Tạo tài khoản học viên mới"}
        </p>
